...
// Configuración WiFi
const char* ssid = "ACZ_22";
const char* password = "asd123as";
...
void configurarTiempo() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    snprintf(fechaActual, sizeof(fechaActual), "%02d/%02d/%04d", 
             rtc.getDay(), rtc.getMonth() + 1, rtc.getYear() + 1900);
    ...
    // Valores por defecto
    strcpy(fechaActual, "01/01/2025");
    strcpy(horaActual, "00:00:00");
  }
}

// Función para inicializar fecha y hora una vez al inicio
void inicializarFechaHora() {
  if (conectarWiFi()) {
    configurarTiempo();
    internetConectado = true;
    Serial.println("Fecha y hora sincronizadas correctamente");
  } else {
    internetConectado = false;
    // Usar valores por defecto si no se puede conectar
    strcpy(fechaActual, "01/01/2025");
    strcpy(horaActual, "00:00:00");
    Serial.println("Usando fecha y hora por defecto");
  }
}
